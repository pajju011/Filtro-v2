    ...
    if (sampleValues.length === 0) {
      ...
      columnTypes[header] =
        typeCounts.number >= typeCounts.date && typeCounts.number >= typeCounts.text
          ? 'number'
          : typeCounts.date >= typeCounts.text
            ? 'date'
            : 'text';
    }
  });
    ...
    return filters.every((filter) => {
      ...
      switch (condition) {
        ...
        case 'between': {
          ...
          return !isNaN(num) && !isNaN(num1) && !isNaN(num2) && num >= num1 && num <= num2;
        }
        ...
        case 'before': {
          ...
          return (
            !isNaN(cellDate.getTime()) && !isNaN(filterDate.getTime()) && cellDate < filterDate
          );
        }
        case 'after': {
          const cellDate = new Date(cellValue);
...
app.post('/api/upload', upload.single('file'), async (req, res) => {
  try {
    ...
    if (req.file.buffer.length > 500 * 1024 * 1024) {
      // > 500MB
    ...
    console.log(
      `Successfully parsed ${parsed.totalRows.toLocaleString()} rows with ${parsed.headers.length} columns`,
    );
...
app.post('/api/export/excel', (req, res) => {
  try {
    ...
    if (rowCount > 1000000) {
      console.warn(
        `Very large dataset export: ${rowCount.toLocaleString()} rows. This may take several minutes...`,
      );
    }
    ...
    for (let i = 0; i < data.length; i += BATCH_SIZE) {
      ...
      if (rowCount > 500000 && processedRows % 100000 === 0) {
        console.log(
          `Export progress: ${processedRows.toLocaleString()} / ${rowCount.toLocaleString()} rows`,
        );
      }
    }
...
app.post('/api/export/pdf', (req, res) => {
  try {
    ...
    doc
      .fontSize(10)
      .text(`Total Rows: ${data.length} | Exported: ${rowsToExport}`, { align: 'center' });
    doc.moveDown();
    ...
    headers.forEach((header, i) => {
      ...
      doc.text(header.substring(0, 15), x + 5, tableTop + 5, {
    ...
    exportData.forEach((row, _rowIndex) => {
    ...
    if (data.length > rowsToExport) {
      doc.moveDown();
      doc.text(
        `(Showing first ${rowsToExport} of ${data.length} rows. Use Excel export for full dataset.)`,
        { align: 'center' },
      );
    }
    ...
    if (error.message && (error.message.includes('heap') || error.message.includes('memory'))) {
      return res.status(413).json({
        error:
          'Dataset too large to export as PDF. Please reduce the number of rows or use Excel export.',
      });
    }
...
app.post('/api/reference-filter', (req, res) => {
  try {
    ...
    if (!referenceData || !Array.isArray(referenceData) || referenceData.length === 0) {
      return res
        .status(400)
        .json({ error: 'Reference data must be provided as a non-empty array' });
    }
    ...
    for (const keyCol of keyColumns) {
      ...
      if (!primaryHeaders.includes(keyCol.primaryColumn)) {
        return res
          .status(400)
          .json({ error: `Primary column "${keyCol.primaryColumn}" not found` });
      }
    }

    console.log(
      `Reference Filter: Ref(${referenceData.length} rows) + Primary(${primaryData.length} rows), Keys: ${keyColumns.length}, Join: ${joinType}`,
    );
...
function validateFilterSchema(filters) {
  ...
  return filters.every((filter) => {
    ...
    if (
      !noValueConditions.has(filter.condition) &&
...
function evaluateFilter(row, filter) {
  ...
  switch (condition) {
    ...
    case 'before': {
      const cellDate = new Date(cellValue);
      const filterDate = new Date(value);
      return !isNaN(cellDate.getTime()) && !isNaN(filterDate.getTime()) && cellDate < filterDate;
    }
    case 'after': {
      const cellDate = new Date(cellValue);
      const filterDate = new Date(value);
